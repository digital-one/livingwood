...
function get_retina_src($src){
$filename = str_replace('.','@2x.',basename($src));
$start = strpos($src,'wp-content');
$path = substr($src,$start);
$path_exists  = './'.str_replace(basename($src),$filename,$path);
$retina_src =  str_replace(basename($src),$filename,$src);
